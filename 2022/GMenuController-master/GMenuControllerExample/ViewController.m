...
- (void)viewDidLoad {
    ...
    GMenuItem *item6 = [[GMenuItem alloc] initWithTitle:@"文字444" target:self action:@selector(test)];
    GMenuItem *item7 = [[GMenuItem alloc] initWithTitle:@"海报分享" target:self action:@selector(test)];
    GMenuItem *item8 = [[GMenuItem alloc] initWithTitle:@"搜索" target:self action:@selector(test)];
...
- (IBAction)btn6:(UIButton*)sender {
    ...
    [GMenuController sharedMenuController].menuViewContainer.cornerRadius = sender.frame.size.height * 0.5;
//    [GMenuController sharedMenuController].menuViewContainer.
    [[GMenuController sharedMenuController] setMenuItems:self.arr5];
    [[GMenuController sharedMenuController] setTargetRect:sender.frame inView:self.view];
    [[GMenuController sharedMenuController] setMenuVisible:YES];
}
