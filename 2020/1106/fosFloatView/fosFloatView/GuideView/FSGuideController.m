...
#import "FSGuideButton.h"
#import "FSGuideIndicatorLineView.h"
#import "FSGuideAppleAnnotationView.h"

@interface FSGuideController ()

@property(nonatomic, weak) id<FSGuideViewControllerDataSource, FSGuideViewControllerDelegate> dataSource;
...
@implementation FSGuideController
...
- (void)showGuideAtIndex:(NSUInteger)index {
    ...
    FSGuideItem *item = [self.dataSource guideViewController:self.identifier itemForGuideAtIndex:index];
    if (index == lastIndex) {
    ...
    // annotationView
    UIView<FSGuideAnnotationViewProtocol> *oldAnnotationView = [self.view viewWithTag:NSNumber.tagOfAnnotationView];
    UIView<FSGuideAnnotationViewProtocol> *annotationView = nil;
    ...
    if (!annotationView) {
        annotationView = FSGuideAppleAnnotationView.new;
    }
    ...
    if (!indicatorView) {
        indicatorView = FSGuideIndicatorLineView.new;
    }
...
+ (void)showsWithDataSource:(id<FSGuideViewControllerDataSource,FSGuideViewControllerDelegate>)dataSource {
    [FSGuideController showsWithDataSource:dataSource identifier:nil];
}

+ (void)showsWithDataSource:(id<FSGuideViewControllerDataSource,FSGuideViewControllerDelegate>)dataSource identifier:(NSString *)identifier {
    [FSGuideController.new showsWithDataSource:dataSource identifier:identifier];
}
...
//MARK: - Getter & Setter
- (void)setDataSource:(id<FSGuideViewControllerDataSource,FSGuideViewControllerDelegate>)dataSource {
...
- (UIButton *)skipButton {
    if (!_skipButton) {
        ...
        FSGuideButton *button = [FSGuideButton buttonWithType:UIButtonTypeSystem];
        _skipButton = button;
