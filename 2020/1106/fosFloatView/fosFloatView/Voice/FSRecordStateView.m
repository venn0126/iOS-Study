...
@implementation FSRecordStateView
{
    NSInteger _allCount;
}
...
- (void)startMeterTimer {
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] > 10.0) {
        ...
        self.levelTimer.preferredFramesPerSecond = 6;
    }
...
- (void)startAudioTimer {
    ...
    if (_recordState != FSRecordStatePlay) {
        _recordDuration = 0;
    }
...
- (void)addSeconed {
    if (_recordState == FSRecordStatePlay ) {
        if (_recordDuration == [FSRecordModel shareInstance].duration) {
            [self.audioTimer invalidate];
            return;
        }
    }
...
- (void)updateMeter {
    CGFloat level = [[FSRecorder shareInstance] levels];
...
- (UILabel *)timeLabel {
    if (_timeLabel == nil) {
        UILabel *timeL = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 50, self.height)];
        timeL.text = @"0:00";
        timeL.textAlignment = NSTextAlignmentCenter;
        timeL.font = [UIFont systemFontOfSize:17];
        timeL.textColor = RGB(119, 119, 119);
...
- (CAShapeLayer *)levelLayer {
    if (_levelLayer == nil) {
        ...
        layer.frame = CGRectMake(self.timeLabel.fs_right, 10, self.width / 2.0 - 30, self.height - 20);
//        layer.backgroundColor = [UIColor whiteColor].CGColor;
        layer.strokeColor = RGB(253, 99, 9).CGColor;
...
//    [self.activityView sizeToFit];
    self.activityView.fs_right = label.fs_left - 5;
    self.activityView.centerY = label.centerY;
    self.activityView.transform = CGAffineTransformMakeScale(0.8, 0.8);
}

#pragma mark - setter
- (void)setRecordState:(FSRecordState)recordState {
    
    self.levelContentView.hidden = YES;
    _recordState = recordState;
    [self.activityView stopAnimating];
    switch (recordState) {
        case FSRecordStateDefault:
            self.titleLb.text = @"按住说话";
            [self updateLableFrame:self.titleLb];
            break;
        case FSRecordStateClickRecord:
            self.titleLb.text = @"点击录音";
            [self updateLableFrame:self.titleLb];
            break;
        case FSRecordStateTouchChangeVoice:
            self.titleLb.text = @"按住变声";
            [self updateLableFrame:self.titleLb];
            break;
        case FSRecordStateListen:
            self.titleLb.text = @"松手试听";
            [self updateLableFrame:self.titleLb];
            break;
        case FSRecordStateCancel:
            self.titleLb.text = @"松手取消发送";
            [self updateLableFrame:self.titleLb];
            break;
        
        case FSRecordStateSend:
            
            break;
        case FSRecordStatePrepare:
            self.titleLb.text = @"准备中";
            [self updateLableFrame:self.titleLb];
            [self.activityView startAnimating];
        
            break;
        case FSRecordStateRecording:
            self.titleLb.hidden = YES;
            self.levelContentView.hidden = NO;
            break;
        case FSRecordStatePlay:
            self.titleLb.hidden = YES;
            self.levelContentView.hidden = NO;
            [self playAndMertering];
            break;
        case FSRecordStatePreparePlay:
            self.titleLb.hidden = YES;
            self.levelContentView.hidden = NO;
            [self preparePlay];
            break;
...
//    NSLog(@"%@",self.allLevels);
    [FSRecordModel shareInstance].path = [FSRecorder shareInstance].recordPath;
    [FSRecordModel shareInstance].levels = [NSArray arrayWithArray:self.allLevels];
    [FSRecordModel shareInstance].duration = (NSTimeInterval)_recordDuration;
...
- (void)preparePlay {
    ...
    [self.allLevels removeAllObjects];
    self.allLevels = [[FSRecordModel shareInstance].levels mutableCopy];
...
//    NSLog(@"%@-----%@",self.allLevels,self.currentLevels);
    _recordDuration = [FSRecordModel shareInstance].duration;
...
- (void)startPlayTimer {
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] > 10.0) {
        ...
        self.playTimer.preferredFramesPerSecond = 6;
    }
