...
//static
static GTFloatButton *_floatWindow;
...
@interface GTFloatButton ()

//悬浮的按钮
@property (nonatomic, strong) GTFloatButtonContent *floatBtn;

@end

@implementation GTFloatButton{
    
    GTAssistiveTouchType  _type;
    //拖动按钮的起始坐标点
    CGPoint _touchPoint;
...
- (GTFloatButtonContent *)floatBtn{
    if (!_floatBtn) {
        _floatBtn = [[GTFloatButtonContent alloc] init];
...
+ (void)show{
    [self showWithType:GTAssistiveTypeNearRight];
}
...
+ (void)showWithType:(GTAssistiveTouchType)type{
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{

        _floatWindow = [[GTFloatButton alloc] initWithType:type frame:CGRectZero];
...
- (instancetype)initWithType:(GTAssistiveTouchType)type
                       frame:(CGRect)frame{
