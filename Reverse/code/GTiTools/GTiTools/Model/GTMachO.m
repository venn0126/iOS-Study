...
#define GTEndianConvert(big, value) \
((big) ? OSSwapInt32((value)) : (value))

@implementation GTMachO
...
{
    if (self = [super init]) {
        uint32_t magic = [handle gt_staticReadUint32];
...
- (void)setupMachO:(NSFileHandle *)handle
{
    ...
    // 读取头部数据
    [handle gt_readData:&header length:headerLength];
    uint32_t cputype = GTEndianConvert(bigEndian, header.cputype);
    uint32_t cpusubtype = GTEndianConvert(bigEndian, header.cpusubtype);
    ...
    // lc的数量
    uint32_t ncmds = GTEndianConvert(bigEndian, header.ncmds);
    // 遍历lc
    for (int i = 0; i < ncmds; i++) {
        struct load_command lc;
        [handle gt_staticReadData:&lc length:sizeof(struct load_command)];
        
        if (lc.cmd == LC_ENCRYPTION_INFO || lc.cmd == LC_ENCRYPTION_INFO_64) {
            struct encryption_info_command eic;
            [handle gt_readData:&eic length:sizeof(struct encryption_info_command)];
...
- (void)setupFat:(NSFileHandle *)handle
{
    ...
    [handle gt_readData:&header length:sizeof(struct fat_header)];
    BOOL bigEndian = (header.magic == FAT_CIGAM);
    
    // 架构数量
    uint32_t archCount = GTEndianConvert(bigEndian, header.nfat_arch);
    NSMutableArray *machOs = [NSMutableArray arrayWithCapacity:archCount];
    
    for (int i = 0; i < archCount; i++) {
        // 读取一个架构的元数据
        struct fat_arch arch;
        [handle gt_readData:&arch length:sizeof(struct fat_arch)];
        // 保留偏移
        unsigned long long archMetaOffset = handle.offsetInFile;
        
        // 偏移到架构具体数据的开始
        [handle seekToFileOffset:GTEndianConvert(bigEndian, arch.offset)];
        GTMachO *machO = [[[self class] alloc] init];
